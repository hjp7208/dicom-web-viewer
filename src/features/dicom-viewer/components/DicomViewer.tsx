...
export default function DicomViewer() {
  ...
  return (
    <div className={`flex-1 w-full relative flex flex-col h-full ${isDark ? "bg-neutral-900" : "bg-slate-100"}`}>
      {loadedSeries.length === 0 ? (
        <div
          className={`flex-1 flex flex-col items-center justify-center p-12 transition-colors duration-300 border-2 border-dashed m-2 ${isDragging
          ...
          {(isParsing || isUnzipping) && (
            <div className="w-full max-w-md mb-8">
              <div className={`w-full rounded-full h-2.5 overflow-hidden ${isDark ? "bg-neutral-800" : "bg-slate-200"}`}>
                <div
                  className={`h-full rounded-full transition-all duration-300 ease-out ${isUnzipping && uploadProgress <= 15 ? 'bg-blue-500' : 'bg-blue-600'
                    }`}
          ...
          {(!isParsing && !isUnzipping) && (
            <div className="flex items-center gap-4">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-lg font-medium transition-colors shadow-lg ${isDark ? "bg-neutral-700 hover:bg-neutral-600 text-white" : "bg-slate-200 hover:bg-slate-300 text-slate-900"
                  }`}
              >
