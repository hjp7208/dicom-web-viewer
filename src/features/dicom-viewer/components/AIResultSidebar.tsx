    ...
    <div className={`w-80 border-l flex flex-col h-full overflow-hidden ${isDark ? "bg-neutral-900 border-neutral-800" : "bg-white border-slate-200"}`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {currentAiResults.map((result, idx) => (
          <div
            ...
            className={`relative w-full aspect-square bg-black border-2 rounded-lg cursor-pointer overflow-hidden transition-colors ${currentAiIdx === idx
                ? 'border-red-500'
      ...
      <div className={`p-4 border-t shrink-0 ${isDark ? "border-neutral-800 bg-neutral-950" : "border-slate-200 bg-slate-50"}`}>
        <div className="mb-4">
          ...
          >
            <div
              className={`flex items-center h-6 w-11 shrink-0 rounded-full px-0.5 transition-colors duration-300 ease-in-out ${showAiOverlay ? 'bg-red-500' : isDark ? 'bg-neutral-600' : 'bg-slate-300'
                }`}
            >
              <div
                className={`w-5 h-5 rounded-full bg-white transition-transform duration-300 ease-in-out shadow-sm ${showAiOverlay ? 'translate-x-5' : 'translate-x-0'
                  }`}
              />
            </div>
        ...
        <div className={`flex items-center justify-center gap-4 rounded-lg p-2 border ${isDark ? "bg-neutral-900 border-neutral-800" : "bg-white border-slate-200"
          }`}>
          <button
          ...
          <div className={`flex items-center gap-2 text-sm ${isDark ? "text-neutral-400" : "text-slate-500"}`}>
            <input
              ...
              className={`w-8 text-center border rounded py-1 focus:outline-none focus:border-blue-500 disabled:opacity-50 ${isDark ? "bg-neutral-800 border-neutral-700 text-white" : "bg-slate-100 border-slate-200 text-slate-900"
                }`}
            />
      ...
      {/* Findings Memo */}
      <div className={`h-1/3 min-h-[250px] border-t p-4 flex flex-col ${isDark ? "border-neutral-800 bg-neutral-900" : "border-slate-200 bg-white"
        }`}>
        <div className="flex items-center justify-between mb-3">
          <h3 className={`font-semibold text-sm flex items-center gap-2 ${isDark ? "text-neutral-100" : "text-slate-800"}`}>
            <span className={`flex items-center justify-center w-6 h-6 rounded-md ${isDark ? "bg-blue-500/15 text-blue-400" : "bg-blue-50 text-blue-600"
              }`}>
          ...
          <button
            ...
            className={`text-xs px-2.5 py-1.5 rounded-md transition-colors font-medium border ${isDark
        ...
        <textarea
          ...
          className={`flex-1 w-full rounded-lg p-3 text-sm resize-none focus:outline-none transition-colors border ${isDark
